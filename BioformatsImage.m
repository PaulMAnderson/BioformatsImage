            ...
            if nargin > 0
                if nargin > 2
                 error('BioformatsImage:TooManyInputArguments',...
                        'Too many input arguments. Was expecting one or two.') 
                elseif nargin == 2
                    obj.tileCZI = varargin{2};
    ...
    methods (Access = private)
        ...
        function obj = getReader(obj)
            ...
            if ~isempty(obj.filename)
                % Custom code to load .czi as individual tiles
                if contains(obj.filename,'.czi') && ~obj.tileCZI
                    ...
                    obj.bfReader.setMetadataOptions(m);

                    % Initialize the reader with the file
                    obj.bfReader.setId(obj.filename);
                else
